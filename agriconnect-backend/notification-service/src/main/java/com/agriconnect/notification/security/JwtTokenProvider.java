...
public class JwtTokenProvider {
    ...
    public boolean validateToken(String t) {
        try { Jwts.parser().verifyWith(secretKey).build().parseSignedClaims(t); return true; }
        catch (JwtException e) { log.warn("Token invalide: {}", e.getMessage()); return false; } }
    public JwtClaims parseToken(String t) {
        Claims c = Jwts.parser().verifyWith(secretKey).build().parseSignedClaims(t).getPayload();
